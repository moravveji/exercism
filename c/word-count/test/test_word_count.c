...
static void test_is_digit() {
   for (char c='0'; c<='9'; c++) {
      TEST_ASSERT(is_digit(c) == 1);
   }

   TEST_ASSERT(is_digit('a') == 0);
   TEST_ASSERT(is_digit('z') == 0);
   TEST_ASSERT(is_digit('A') == 0);
   TEST_ASSERT(is_digit('Z') == 0);
   TEST_ASSERT(is_digit('!') == 0);
}

static void test_is_delimiter() {
  TEST_ASSERT(is_delimiter(' ') == 1);
  TEST_ASSERT(is_delimiter(',') == 1);
  TEST_ASSERT(is_delimiter('\n') == 1);

  TEST_ASSERT(is_delimiter('\0') == 0);
  TEST_ASSERT(is_delimiter('u') == 0);
}

static void test_store_word() {
   // TEST_IGNORE();
   word_count_word_t * words;
   words = (word_count_word_t *) malloc(3 * sizeof(word_count_word_t));
   char man[MAX_WORD_LENGTH] = "man";
   char woman[MAX_WORD_LENGTH] = "woman";
   int wc = 0;
   store_word(words, man, &wc, strlen(man));
   store_word(words, woman, &wc, strlen(woman));
   store_word(words, woman, &wc, strlen(woman));
   TEST_ASSERT_EQUAL(wc, 2);
   TEST_ASSERT_EQUAL_STRING_MESSAGE(words[0].text, man, "man failed");
   TEST_ASSERT_EQUAL_STRING_MESSAGE(words[1].text, woman, "woman failed");
   TEST_ASSERT_EQUAL(words[0].count, 1);
   TEST_ASSERT_EQUAL(words[1].count, 2);
}
...
{
   if (DEBUG) printf("check_solution: expected_word_count=%d, actual_word_count=%d\n", \
...
static void test_word_count_one_word(void)
{
   ...
   if (DEBUG) printf("test_word_count_one_word: %d\n", actual_word_count);
...
int main(void)
{
   ...
   RUN_TEST(test_is_delimiter);
   RUN_TEST(test_is_digit);
   RUN_TEST(test_store_word);
